...
## Протокол challenge-response (реализован)

Двухключевая схема:

- **Ed25519 сервера** — подписывает лицензию; клиент проверяет офлайн.
- **Ed25519 устройства** (сервер панели) — доказывает, что запрос исходит
  от владельца fingerprint; сервер проверяет онлайн.

```
Клиент                                        Сервер лицензий
  │                                                │
  │ 1. POST /api/v1/challenge {fingerprint}        │
  │ ──────────────────────────────────────────────►│  создаёт nonce (TTL 120с, single-use)
  │ ◄──────────── {nonce} ─────────────────────────│
  │                                                │
  │ 2. message = "{nonce}|{fingerprint}|{license_key}"
  │    device_sig = Ed25519_sign(device_priv, message)
  │                                                │
  │ 3. POST /api/v1/activate                        │
  │    {license_key, fingerprint, nonce,            │
  │     device_pubkey, device_sig}                 │
  │ ──────────────────────────────────────────────►│  4. nonce валиден? (не использован, не истёк)
  │                                                │  5. device_sig верна? (проверка device_pubkey)
  │                                                │  6. fingerprint ↔ device_pubkey привязка
  │                                                │  7. лимит активаций, срок, статус
  │ ◄── {license + signature(server_priv)} ────────│  8. подпись сервера
  │                                                │
  │ 9. verify(server_pub, license) офлайн          │
  │ 10. сохранить /opt/license.json (0600)         │
```

Реплей и подмена ключа устранены: nonce одноразовый, а `device_pubkey`
привязывается к fingerprint при первой активации и далее должен совпадать.

### Проверено тестами

| Сценарий | Результат |
|---|---|
| Валидный challenge-response | 200 OK |
| Повторное использование nonce | 401 |
| Неверная подпись устройства | 401 |
| Неизвестный/просроченный nonce | 401 |
| Кража ключа на другом сервере | 409 (лимит/привязка) |
| Подмена device-ключа при том же fingerprint | 409 |
| Валидная лицензия на клиенте | подпись True |
| Подделка полей лицензии | подпись False |
