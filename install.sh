...
cd "$INSTALL_DIR" && python3 -c "import sys; sys.path.insert(0, '$INSTALL_DIR'); import app; app.generate_dialplan_from_tree(); app.generate_pjsip_conf(); app.generate_queues_conf(); app.generate_voicemail_conf(); app.generate_meetme_conf()" 2>/dev/null || echo "   [!] Автогенерация диалплана будет выполнена при первом сохранении настроек в UI."
