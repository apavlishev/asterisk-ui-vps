...
def api_amocrm_users():
    ...
    token = amo.get('token', '').strip()
    if not subdomain or not token:
        return jsonify({'status': 'error', 'message': 'Субдомен или токен amoCRM не настроены', 'users': []})
    ...
    try:
        ...
        if r.status_code == 200:
            raw_users = r.json().get('_embedded', {}).get('users', [])
            users = [{'id': u.get('id'), 'name': u.get('name'), 'email': u.get('email')} for u in raw_users]
            return jsonify({'status': 'ok', 'users': users})
        return jsonify({'status': 'error', 'message': f'Ошибка amoCRM [{r.status_code}]: {r.text}', 'users': []})
    except Exception as e:
...
def toggle_modems_test_mode():
    ...
    save_integrations(cfg)
    flash(f"Тестовый режим модемов {'включен' if enabled else 'выключен'}!")
    return redirect(url_for('index'))
...
def save_amocrm_seats():
    ...
    mapping = {}
    for k, v in request.form.items():
        if k.startswith('seat_') and v:
            exten = k.replace('seat_', '')
            mapping[exten] = int(v)
            
    default_seat = request.form.get('seat_default')
    if default_seat:
        mapping['default'] = int(default_seat)
        
    cfg['amocrm']['user_mapping'] = mapping
    save_integrations(cfg)
    flash('Посадочные места amoCRM успешно сохранены!')
    return redirect(url_for('index'))

@app.route('/settings/ring-groups/add', methods=['POST'])
def add_ring_group():
    exten = request.form.get('group_exten', '').strip()
    name = request.form.get('group_name', '').strip()
    members = request.form.getlist('group_members')
    timeout = int(request.form.get('group_timeout', 30))
    
    if not exten or not name or not members:
        flash('Ошибка: укажите номер группы, название и выберите хотя бы одного участника.')
        return redirect(url_for('index'))
        
    cfg = load_integrations()
    if 'ring_groups' not in cfg:
        cfg['ring_groups'] = []
        
    # Удаляем дубликат, если номер уже был
    cfg['ring_groups'] = [g for g in cfg['ring_groups'] if str(g.get('exten')) != exten]
    cfg['ring_groups'].append({
        'exten': exten,
        'name': name,
        'members': members,
        'timeout': timeout,
    ...
    save_integrations(cfg)
    generate_dialplan_from_tree()
    flash(f'Группа / Отдел {name} ({exten}) успешно создана!')
    return redirect(url_for('index'))

@app.route('/settings/ring-groups/delete/<exten>', methods=['POST'])
def delete_ring_group(exten):
    cfg = load_integrations()
    if 'ring_groups' in cfg:
        cfg['ring_groups'] = [g for g in cfg['ring_groups'] if str(g.get('exten')) != str(exten)]
        save_integrations(cfg)
        generate_dialplan_from_tree()
    flash(f'Группа {exten} удалена!')
    return redirect(url_for('index'))
...
def add_sip_trunk():
    ...
    cfg = load_integrations()
    if 'sip_trunks' not in cfg:
        cfg['sip_trunks'] = []

    # Генерируем уникальный slug транка
    clean_name = re.sub(r'[^a-zA-Z0-9_]', '_', name.lower())
    trunk_id = f"trunk_{clean_name}_{int(time.time()) % 10000}"
    
    cfg['sip_trunks'].append({
        'id': trunk_id,
        'name': name,
        'host': host,
        'port': port,
    ...
    save_integrations(cfg)
    generate_pjsip_conf()
    flash(f'Внешняя телефония / SIP-транк «{name}» успешно подключен!')
    return redirect(url_for('index'))

@app.route('/settings/sip-trunks/toggle/<trunk_id>', methods=['POST'])
def toggle_sip_trunk(trunk_id):
    cfg = load_integrations()
    if 'sip_trunks' in cfg:
        for t in cfg['sip_trunks']:
            if t.get('id') == trunk_id:
                t['enabled'] = not t.get('enabled', True)
                break
        save_integrations(cfg)
        generate_pjsip_conf()
    flash('Статус транка обновлен!')
    return redirect(url_for('index'))
...
def delete_sip_trunk(trunk_id):
    cfg = load_integrations()
    if 'sip_trunks' in cfg:
        cfg['sip_trunks'] = [t for t in cfg['sip_trunks'] if t.get('id') != trunk_id]
        save_integrations(cfg)
        generate_pjsip_conf()
    flash(f'SIP-транк удален!')
    return redirect(url_for('index'))




@app.route('/settings/outbound-routes/add', methods=['POST'])
def add_outbound_route():
    name = request.form.get('route_name', '').strip()
    prefix = request.form.get('route_prefix', '').strip()
    gw_primary = request.form.get('gw_primary', 'dongle0').strip()
    gw_failover = request.form.get('gw_failover', '').strip()

    if not name or not prefix or not gw_primary:
        flash('Укажите название, префикс/маску и основной шлюз!')
        return redirect(url_for('index'))

    cfg = load_integrations()
    if 'outbound_routing' not in cfg:
        cfg['outbound_routing'] = {'routes': [], 'operator_rules': {}, 'enable_failover': True}

    route_id = f"route_{re.sub(r'[^a-zA-Z0-9_]', '_', name.lower())}_{int(time.time()) % 10000}"
    gateways = [gw_primary]
    if gw_failover and gw_failover != gw_primary:
        gateways.append(gw_failover)

    cfg['outbound_routing']['routes'].append({
        'id': route_id,
        'name': name,
        'prefix': prefix,
        'gateways': gateways,
    ...
    save_integrations(cfg)
    generate_dialplan_from_tree()
    flash(f'Исходящий маршрут «{name}» успешно добавлен!')
    return redirect(url_for('index'))

@app.route('/settings/outbound-routes/delete/<route_id>', methods=['POST'])
def delete_outbound_route(route_id):
    cfg = load_integrations()
    if 'outbound_routing' in cfg and 'routes' in cfg['outbound_routing']:
        cfg['outbound_routing']['routes'] = [r for r in cfg['outbound_routing']['routes'] if r.get('id') != route_id]
        save_integrations(cfg)
        generate_dialplan_from_tree()
    flash('Маршрут удален!')
    return redirect(url_for('index'))

@app.route('/settings/outbound-operators', methods=['POST'])
def save_outbound_operators():
    cfg = load_integrations()
    if 'outbound_routing' not in cfg:
        cfg['outbound_routing'] = {'routes': [], 'operator_rules': {}, 'enable_failover': True}
    ...
    op_rules = {}
    for k, v in request.form.items():
        if k.startswith('op_gw_') and v:
            exten = k.replace('op_gw_', '')
            op_rules[exten] = v

    cfg['outbound_routing']['operator_rules'] = op_rules
    cfg['outbound_routing']['enable_failover'] = True if request.form.get('enable_failover') else False

    save_integrations(cfg)
    generate_dialplan_from_tree()
    flash('Настройки исходящих линий операторов сохранены!')
    return redirect(url_for('index'))

def initiate_callto(operator, phone, callerid=None):
    if not operator:
        operator = "101"
    if not phone:
        return False, "Не указан номер телефона клиента"

    # Очистка номеров
    clean_op = re.sub(r'[^0-9a-zA-Z_]', '', str(operator).strip())
    clean_dst = str(phone).strip().replace(" ", "").replace("-", "").replace("(", "").replace(")", "")
    if not clean_dst.startswith("+") and not clean_dst.isdigit():
        clean_dst = re.sub(r'[^0-9+]', '', clean_dst)

    cid_str = callerid or f"CallTo: {clean_dst} <{clean_op}>"
    
    # Формируем и выполняем команду Asterisk Originate
    # 1 плечо: софтфон оператора (Local/{clean_op}@from-internal)
    # 2 плечо: набор номера клиента через диалплан (extension {clean_dst}@from-internal)
    cmd = f"channel originate Local/{clean_op}@from-internal extension {clean_dst}@from-internal callerid \"{cid_str}\""
    out = run_asterisk(cmd)
    return True, f"Вызов инициирован: звоним оператору {clean_op}, затем соединяем с {clean_dst}"


@app.route('/api/v1/callto', methods=['GET', 'POST'])
@app.route('/callto', methods=['GET', 'POST'])
def api_callto():
    cfg = load_integrations()
    webhooks_cfg = cfg.get('webhooks', {})
    
    # 1. Проверка активности Webhooks API
    if not webhooks_cfg.get('enabled', True):
        return jsonify({"status": "error", "error": "CallTo Webhook API отключен в настройках"}), 403

    # 2. Получение параметров из JSON / Form / Query Params
    req_token = request.headers.get('X-API-Key') or request.headers.get('Authorization')
    ...
    # Дополняем Query params
    for k, v in request.args.items():
        if k not in params:
            params[k] = v

    token = req_token or params.get('token') or params.get('api_key')
    valid_token = webhooks_cfg.get('api_token')

    # Внутренняя проверка (если вызов из локальной веб-панели без токена)
    is_local_ui = request.referrer and (request.host in request.referrer)
    if not is_local_ui:
        if not token or token != valid_token:
            return jsonify({"status": "error", "error": "Неверный или отсутствующий API токен (X-API-Key / token)"}), 401

    operator = params.get('operator') or params.get('src') or params.get('from') or webhooks_cfg.get('default_operator', '101')
    phone = params.get('phone') or params.get('dst') or params.get('to') or params.get('number') or params.get('client')
    callerid = params.get('callerid')

    if not phone:
        return jsonify({"status": "error", "error": "Параметр phone (номер телефона) обязателен"}), 400

    ok, msg = initiate_callto(operator, phone, callerid)
    if ok:
        # Если вызов по прямой ссылке из браузера (GET HTML), покажем красивую страницу подтверждения
        if request.method == 'GET' and 'text/html' in request.headers.get('Accept', '') and not request.args.get('format') == 'json':
            return f"""
            <html><head><meta charset="utf-8"><title>CallTo Вызов</title>
            <style>body{{background:#0b0f19; color:#f8fafc; font-family:sans-serif; text-align:center; padding-top:80px;}}
            .box{{max-width:450px; margin:0 auto; background:#111827; padding:30px; border-radius:12px; border:1px solid #1f2937;}}
            h2{{color:#34d399;}} p{{color:#94a3b8; font-size:14px;}}
        /* СТИЛИ ПАГИНАЦИИ ТАБЛИЦЫ ВЫЗОВОВ */
        .pagination-container {
            display: flex;
            justify-content: space-between;
        ...
        .page-btn:disabled {
            ...
            <div class="box">
                <h2>📞 Вызов инициирован!</h2>
                <p>Asterisk звонит оператору <b>{operator}</b>.</p>
                <p>После снятия трубки начнется набор клиенту <b>{phone}</b>.</p>
                <p style="font-size:12px; color:#64748b;">(Это окно закроется автоматически через 5 сек)</p>
            </div>
...
@app.route('/settings/webhooks/save', methods=['POST'])
def save_webhooks_settings():
    cfg = load_integrations()
    if 'webhooks' not in cfg:
        cfg['webhooks'] = {}
    
    cfg['webhooks']['enabled'] = bool(request.form.get('webhooks_enabled'))
    cfg['webhooks']['default_operator'] = request.form.get('default_operator', '101').strip()
    
    new_tok = request.form.get('api_token', '').strip()
    if new_tok:
        cfg['webhooks']['api_token'] = new_tok

    save_integrations(cfg)
    return redirect('/?tab=integrations')


@app.route('/settings/webhooks/regenerate-token', methods=['POST'])
def regenerate_webhooks_token():
    cfg = load_integrations()
    if 'webhooks' not in cfg:
        cfg['webhooks'] = {}
    cfg['webhooks']['api_token'] = 'sk_' + secrets.token_hex(16)
    save_integrations(cfg)
    return redirect('/?tab=integrations')


@app.route('/settings/network/quick-save', methods=['POST'])
def quick_save_network():
    ip_addr = request.form.get('ip_addr', '').strip()
    prefix = int(request.form.get('prefix', 24))
    ...
    cfg['network']['saved_ip'] = ip_addr
    cfg['network']['saved_prefix'] = prefix
    cfg['network']['saved_gateway'] = gateway
    cfg['network']['mode'] = 'static'
    save_integrations(cfg)
    
    ok, msg = apply_network_settings(ip_addr, prefix, gateway, mode='static')
    flash(f"IP {ip_addr} успешно зафиксирован! {msg if not ok else ''}")
    return redirect('/?tab=network')


@app.route('/settings/network/save', methods=['POST'])
def save_network_settings():
    mode = request.form.get('net_mode', 'static')
    ip_addr = request.form.get('ip_addr', '').strip()
    prefix = int(request.form.get('prefix', 24))
    gateway = request.form.get('gateway', '').strip()
    ...
    if mode == 'static' and ip_addr:
        ...
        cfg['network']['saved_ip'] = None
        save_integrations(cfg)
        ok, msg = apply_network_settings(None, None, None, mode='dhcp')
        
    flash("Сетевые настройки успешно обновлены!")
    return redirect('/?tab=network')



def self_heal_pbx_environment():
    try:
        os.makedirs(RECORD_DIR, exist_ok=True)
        os.chmod(RECORD_DIR, 0o777)
        if not os.path.exists(EXTENSIONS_CONF) or 'MixMonitor' not in open(EXTENSIONS_CONF, 'r', encoding='utf-8', errors='ignore').read():
            generate_dialplan_from_tree()
    except Exception as e:
