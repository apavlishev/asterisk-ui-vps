...
import csv
import requests
from flask import Flask, make_response, render_template_string, render_template, request, redirect, url_for, flash, jsonify, send_from_directory, abort, session, Response
...
# ================= ACD QUEUES (queues.conf) =================
QUEUES_CONF = '/etc/asterisk/queues.conf'


def get_queues():
    cfg = load_integrations()
    return cfg.get('queues', [])
...
def generate_queues_conf():
    """Writes /etc/asterisk/queues.conf from the configured queues + members."""
    queues = get_queues()
    out = [
        "; ==========================================================",
        "; Queues configuration (Auto-generated by Asterisk PBX Web)",
        "; ==========================================================",
        "",
        "[general]",
    ...
    for q in queues:
        number = str(q.get('exten', '')).strip()
        if not number:
            continue
        name = q.get('name', number)
        strategy = q.get('strategy', 'ringall')
        timeout = int(q.get('timeout', 20))
        retry = int(q.get('retry', 5))
        maxlen = int(q.get('maxlen', 0))
        moh = q.get('music_on_hold') or 'default'
        members = q.get('members', [])

        out.append(f"; Очередь: {name} ({number})")
        out.append(f"[{number}]")
        out.append(f"strategy = {strategy}")
        out.append(f"timeout = {timeout}")
        out.append(f"retry = {retry}")
        out.append(f"maxlen = {maxlen}")
        out.append(f"musicclass = {moh}")
        out.append(f"musiconhold = {moh}")
        out.append("announce-holdtime = yes")
        out.append("announce-position = yes")
        out.append("joinempty = yes")
        out.append("leavewhenempty = no")
        out.append(f"queue-youarenext = queue-youarenext")
        out.append(f"queue-thereare = queue-thereare")
        out.append("queue-callswaiting = queue-callswaiting")
        out.append("")
        for m in members:
            ext = m.get('exten') if isinstance(m, dict) else str(m)
            if not ext:
                continue
            penalty = m.get('penalty', 0) if isinstance(m, dict) else 0
            out.append(f"member => PJSIP/{ext},,{penalty}")
        out.append("")

    try:
        with open(QUEUES_CONF, 'w', encoding='utf-8') as f:
            f.write('\n'.join(out))
        run_asterisk('queue reload all')
    except Exception as e:
        print(f"[queues] save failed: {e}")
    return True


@app.route('/api/queues', methods=['GET'])
def api_get_queues():
    return jsonify({'status': 'ok', 'queues': get_queues()})


@app.route('/api/queues/save', methods=['POST'])
def api_save_queue():
    data = request.get_json() or {}
    q_id = data.get('id') or f"queue_{int(time.time())}"
    name = (data.get('name') or '').strip()
    exten = str(data.get('exten') or '').strip()
    if not name or not exten:
        return jsonify({'status': 'error', 'message': 'Название и номер очереди обязательны'})

    members = []
    for m in data.get('members', []):
        if isinstance(m, dict):
            if m.get('exten'):
                members.append({'exten': str(m['exten']), 'penalty': int(m.get('penalty', 0) or 0)})
        elif str(m).strip():
            members.append({'exten': str(m).strip(), 'penalty': 0})
    ...
    entry = {
        ...
        'retry': int(data.get('retry', 5) or 5),
        'maxlen': int(data.get('maxlen', 0) or 0),
        'music_on_hold': (data.get('music_on_hold') or 'default').strip(),
        'members': members,
    }
    updated = False
    for i, q in enumerate(queues):
        if q.get('id') == q_id or str(q.get('exten')) == exten:
            queues[i] = entry
            updated = True
            break
    if not updated:
        queues.append(entry)
    save_queues(queues)
    auth_mgr.audit(session.get('username'), 'queue_save', f"{name} ({exten})")
    return jsonify({'status': 'ok', 'message': 'Очередь сохранена', 'queues': queues})


@app.route('/api/queues/delete', methods=['POST'])
def api_delete_queue():
    data = request.get_json() or {}
    q_id = data.get('id')
    if not q_id:
        return jsonify({'status': 'error', 'message': 'ID очереди не указан'})
    queues = [q for q in get_queues() if q.get('id') != q_id]
    save_queues(queues)
    auth_mgr.audit(session.get('username'), 'queue_delete', str(q_id))
    return jsonify({'status': 'ok', 'queues': queues})


@app.route('/api/queues/status', methods=['GET'])
def api_queues_status():
    """Live queue status: waiting callers, agents, SL/statistics via AMI if possible."""
    queues = get_queues()
    result = []
    for q in queues:
        number = str(q.get('exten', ''))
        raw = run_asterisk(f'queue show {number}')
        waiting = 0
        agents = []
        m = re.search(r'(\d+)\s+calls?\s+waiting', raw or '')
        if m:
            waiting = int(m.group(1))
        if raw:
            for line in raw.splitlines():
                am = re.search(r'^\s*(\d+)\s+has taken\s+(\d+)\s+calls?\s+\(([^)]*)\)\s+\(([^)]*)\)', line)
                if am:
                    agents.append({
                        'member': am.group(1),
                        'calls_taken': int(am.group(2)),
                        'status': am.group(3).strip(),
                        'paused': 'paused' in am.group(3).lower(),
                    })
        result.append({
            'id': q.get('id'),
            'name': q.get('name'),
            ...
            'waiting': waiting,
            'agents': agents,
            'members_count': len(q.get('members', [])),
        })
    return jsonify({'status': 'ok', 'queues': result})


@app.route('/api/queues/agent', methods=['POST'])
def api_queue_agent_action():
    """Pause/unpause/add/remove a queue member at runtime."""
    data = request.get_json() or {}
    action = (data.get('action') or '').strip()
    queue = str(data.get('queue') or '').strip()
    member = str(data.get('member') or '').strip()
    if not queue or not member:
        return jsonify({'status': 'error', 'message': 'Нужны queue и member'})

    interface = member if member.startswith('PJSIP/') else f"PJSIP/{member}"
    if action == 'pause':
        out = run_asterisk(f'queue pause member {interface} queue {queue}')
    elif action == 'unpause':
        out = run_asterisk(f'queue unpause member {interface} queue {queue}')
    elif action == 'remove':
        out = run_asterisk(f'queue remove member {interface} queue {queue}')
    elif action == 'add':
        out = run_asterisk(f'queue add member {interface} to {queue}')
    elif action == 'reload':
        out = run_asterisk('queue reload all')
    else:
        return jsonify({'status': 'error', 'message': f'Неизвестное действие: {action}'})

    auth_mgr.audit(session.get('username'), f'queue_agent_{action}', f'{queue}/{member}')
    return jsonify({'status': 'ok', 'output': out or ''})


# ================= VOICEMAIL (UI + MWI + e-mail) =================
...
def get_voicemail_settings():
    cfg = load_integrations()
    return cfg.get('voicemail', {})


def save_voicemail_settings(vm):
    cfg = load_integrations()
    cfg['voicemail'] = vm
    save_integrations(cfg)
...
def get_voicemail_boxes():
    vm = get_voicemail_settings()
    boxes = vm.get('boxes', [])
    return boxes


def _vm_greeting_dir(context, mailbox):
    return os.path.join(VOICEMAIL_SPOOL, context, mailbox, 'INBOX')


def generate_voicemail_conf():
    """Writes voicemail.conf from the configured boxes + global settings."""
    vm = get_voicemail_settings()
    boxes = vm.get('boxes', [])
    email_enabled = vm.get('email_enabled', False)
    attach = 'yes' if vm.get('email_attach', True) else 'no'
    smtp = vm.get('smtp', {})

    out = [
        "; ==========================================================",
        "; Voicemail configuration (Auto-generated by Asterisk PBX Web)",
        "; ==========================================================",
        "",
        "[general]",
    ...
    if email_enabled:
        out += [
            "emailsubject=Новое голосовое сообщение от ${VM_CALLERID}",
            "emailbody=Вам новое сообщение (${VM_MSGNUM}) от ${VM_CALLERID} длительностью ${VM_DUR}. Номер: ${VM_CALLERID}",
            "emaildateformat=%A, %B %d, %Y at %r",
            "",
        ]
    if smtp:
        out.append("[general-smtp]")
        out.append("; SMTP-параметры задаются в разделе Настройки → Почта")
        out.append("")

    contexts = {}
    for b in boxes:
        ctx = b.get('context', 'default')
        contexts.setdefault(ctx, []).append(b)

    for ctx, list_ in contexts.items():
        out.append(f"[{ctx}]")
        for b in list_:
            ext = str(b.get('exten', '')).strip()
            if not ext:
                continue
            password = b.get('password', '')
            name = b.get('name', f'Абонент {ext}')
            email = b.get('email', '') or ''
            delete_after = 'yes' if b.get('delete_after_email') else 'no'
            out.append(f"{ext} => {password},{name},{email},{delete_after}")
        out.append("")

    try:
        with open(VOICEMAIL_CONF, 'w', encoding='utf-8') as f:
            f.write('\n'.join(out))
        run_asterisk('voicemail reload')
        # Prepare spool dirs + MWI reload
        for b in boxes:
            ext = str(b.get('exten', '')).strip()
            ctx = b.get('context', 'default')
            if not ext:
                continue
            try:
                os.makedirs(os.path.join(VOICEMAIL_SPOOL, ctx, ext, 'INBOX'), exist_ok=True)
                os.makedirs(os.path.join(VOICEMAIL_SPOOL, ctx, ext, 'Old'), exist_ok=True)
                os.makedirs(os.path.join(VOICEMAIL_SPOOL, ctx, ext, 'Urgent'), exist_ok=True)
            except Exception:
                pass
        return True
    except Exception as e:
        print(f"[voicemail] save failed: {e}")
        return False


def list_voicemail_messages(exten=None, context='default'):
    """Scans the Asterisk voicemail spool for messages."""
    result = []
    if not os.path.isdir(VOICEMAIL_SPOOL):
        return result
    contexts = [context] if context else os.listdir(VOICEMAIL_SPOOL)
    for ctx in contexts:
        ctx_path = os.path.join(VOICEMAIL_SPOOL, ctx)
        if not os.path.isdir(ctx_path):
            continue
        for mailbox in sorted(os.listdir(ctx_path)):
            if exten and mailbox != str(exten):
                continue
            mb_path = os.path.join(ctx_path, mailbox)
            if not os.path.isdir(mb_path):
                continue
            for folder in ['INBOX', 'Old', 'Urgent']:
                fpath = os.path.join(mb_path, folder)
                ...
                for fn in sorted(os.listdir(fpath)):
                    if not fn.endswith('.txt'):
                        continue
                    base = fn[:-4]
                    msg_path = os.path.join(fpath, base)
                    info = {}
                    try:
                        with open(os.path.join(fpath, fn), 'r', encoding='utf-8', errors='ignore') as f:
                            for line in f:
                                if '=' in line:
                                    k, v = line.split('=', 1)
                        ...
                        pass
                    audio = None
                    for ext_candidate in ['.wav', '.WAV', '.gsm', '.wav49']:
                        if os.path.exists(msg_path + ext_candidate):
                            audio = base + ext_candidate
                            break
                    try:
                        mtime = os.path.getmtime(msg_path + '.txt')
                    except Exception:
                        mtime = 0
                    result.append({
                        'id': base,
                        ...
                        'origdate': info.get('origdate', ''),
                        'duration': info.get('duration', '0'),
                        'audio': audio,
                        'new': folder == 'INBOX',
                        'mtime': mtime,
                    })
    return sorted(result, key=lambda m: m['mtime'], reverse=True)


def _vm_file_path(mailbox, msg_id, context='default', folder=None):
    """Locates the .txt for a message without allowing path traversal."""
    msg_id = os.path.basename(str(msg_id))
    mailbox = os.path.basename(str(mailbox))
    context = os.path.basename(str(context))
    if folder:
        folder = os.path.basename(str(folder))
        base = os.path.join(VOICEMAIL_SPOOL, context, mailbox, folder, msg_id)
        if os.path.exists(base + '.txt'):
            return base
    for f in ['INBOX', 'Old', 'Urgent']:
        base = os.path.join(VOICEMAIL_SPOOL, context, mailbox, f, msg_id)
        if os.path.exists(base + '.txt'):
            return base
    return None


@app.route('/api/voicemail', methods=['GET'])
def api_voicemail_list():
    exten = request.args.get('exten', '').strip()
    messages = list_voicemail_messages(exten or None)
    return jsonify({
        'status': 'ok',
        'messages': messages,
        'boxes': get_voicemail_boxes(),
        'settings': {k: v for k, v in get_voicemail_settings().items() if k != 'boxes'},
    })


@app.route('/api/voicemail/save', methods=['POST'])
def api_voicemail_save():
    data = request.get_json() or {}
    ...
    for b in boxes:
        ...
        clean.append({
            'exten': ext,
            'name': (b.get('name') or f'Абонент {ext}').strip(),
            'password': str(b.get('password') or (secrets.randbelow(9000) + 1000)),
            'email': (b.get('email') or '').strip(),
            'context': (b.get('context') or 'default').strip(),
            'delete_after_email': bool(b.get('delete_after_email')),
        })
    vm = get_voicemail_settings()
    vm['boxes'] = clean
    vm['email_enabled'] = bool(data.get('email_enabled', vm.get('email_enabled', False)))
    vm['email_attach'] = bool(data.get('email_attach', vm.get('email_attach', True)))
    save_voicemail_settings(vm)
    auth_mgr.audit(session.get('username'), 'voicemail_save', f'{len(clean)} boxes')
    return jsonify({'status': 'ok', 'boxes': clean})


@app.route('/api/voicemail/delete', methods=['POST'])
def api_voicemail_delete():
    data = request.get_json() or {}
    msg_id = str(data.get('id') or '')
    mailbox = str(data.get('mailbox') or '')
    context = str(data.get('context') or 'default')
    if not msg_id or not mailbox:
        return jsonify({'status': 'error', 'message': 'Нужны id и mailbox'})
    base = _vm_file_path(mailbox, msg_id, context, data.get('folder'))
    if not base:
        return jsonify({'status': 'error', 'message': 'Сообщение не найдено'})
    for f in ['.txt', '.wav', '.WAV', '.gsm', '.wav49', '.g722', '.ulaw', '.alaw']:
        try:
            if os.path.exists(base + f):
                os.remove(base + f)
        except Exception:
            pass
    auth_mgr.audit(session.get('username'), 'voicemail_delete', f'{mailbox}/{msg_id}')
    return jsonify({'status': 'ok'})


@app.route('/api/voicemail/move', methods=['POST'])
def api_voicemail_move():
    data = request.get_json() or {}
    msg_id = str(data.get('id') or '')
    mailbox = str(data.get('mailbox') or '')
    context = str(data.get('context') or 'default')
    target = str(data.get('target') or data.get('folder_target') or 'Old')
    if target not in ('INBOX', 'Old', 'Urgent'):
        return jsonify({'status': 'error', 'message': 'Неверная папка'})
    base = _vm_file_path(mailbox, msg_id, context, data.get('folder'))
    if not base:
        return jsonify({'status': 'error', 'message': 'Сообщение не найдено'})
    dest_dir = os.path.join(VOICEMAIL_SPOOL, os.path.basename(context), os.path.basename(mailbox), target)
    os.makedirs(dest_dir, exist_ok=True)
    for f in ['.txt', '.wav', '.WAV', '.gsm', '.wav49', '.g722']:
        src = base + f
...
@app.route('/api/voicemail/audio/<context>/<mailbox>/<folder>/<path:filename>')
def api_voicemail_audio(context, mailbox, folder, filename):
    safe = os.path.join(
        VOICEMAIL_SPOOL, os.path.basename(context), os.path.basename(mailbox),
        os.path.basename(folder), os.path.basename(filename))
    if not os.path.exists(safe):
        abort(404)
    return send_from_directory(os.path.dirname(safe), os.path.basename(safe))


@app.route('/api/voicemail/mwi', methods=['POST'])
def api_voicemail_mwi():
    """Forces an MWI update for all boxes (e.g. after import)."""
    out = run_asterisk('voicemail reload')
...
def get_phonebook():
    cfg = load_integrations()
    return cfg.get('phonebook', {}).get('contacts', [])


def save_phonebook(contacts):
    cfg = load_integrations()
    pb = cfg.get('phonebook', {})
    pb['contacts'] = contacts
    cfg['phonebook'] = pb
    save_integrations(cfg)


def _phonebook_ctx_entries():
    """Contacts formatted as (name, number) for the dialplan/CLI directory."""
    out = []
    for c in get_phonebook():
...
def sync_asterisk_directory():
    """Writes /etc/asterisk/directory.conf (Asterisk Directory app) from the phonebook."""
    entries = _phonebook_ctx_entries()
    lines = [
        "; ==========================================================",
        "; Phonebook / Directory (Auto-generated by Asterisk PBX Web)",
        "; Dial *650 (or the directory code) to look up contacts by name.",
        "; ==========================================================",
        "[default]",
    ]
    for name, num in entries:
        safe_name = name.replace(',', ' ').replace(';', ' ')
        lines.append(f"{num} => {safe_name}")
    lines.append("")
    try:
        with open('/etc/asterisk/directory.conf', 'w', encoding='utf-8') as f:
            f.write('\n'.join(lines))
        run_asterisk('dialplan reload')
    except Exception as e:
        print(f'[phonebook] directory sync failed: {e}')
    return len(entries)


@app.route('/api/phonebook', methods=['GET'])
def api_phonebook_list():
    ...
    if q:
        contacts = [
            ...
            or q in str(c.get('number', '')).lower()
            or q in str(c.get('company', '')).lower()
            or q in str(c.get('email', '')).lower()
        ]
    return jsonify({'status': 'ok', 'contacts': contacts, 'total': len(get_phonebook())})


@app.route('/api/phonebook/save', methods=['POST'])
def api_phonebook_save():
    data = request.get_json() or {}
    cid = str(data.get('id') or '').strip() or f"c_{int(time.time() * 1000)}"
    name = str(data.get('name') or '').strip()
    number = str(data.get('number') or '').strip()
    if not name or not number:
        return jsonify({'status': 'error', 'message': 'Имя и номер обязательны'})
    entry = {
        'id': cid,
        'name': name,
        'number': number,
    ...
    sync_asterisk_directory()
    auth_mgr.audit(session.get('username'), 'phonebook_save', f'{name} <{number}>')
    return jsonify({'status': 'ok', 'contact': entry, 'contacts': contacts})


@app.route('/api/phonebook/delete', methods=['POST'])
def api_phonebook_delete():
    data = request.get_json() or {}
    cid = str(data.get('id') or '')
    contacts = [c for c in get_phonebook() if c.get('id') != cid]
    save_phonebook(contacts)
    sync_asterisk_directory()
    auth_mgr.audit(session.get('username'), 'phonebook_delete', cid)
    return jsonify({'status': 'ok', 'contacts': contacts})


@app.route('/api/phonebook/import', methods=['POST'])
def api_phonebook_import():
    """Imports contacts from CSV text (name,number,company,email per line)."""
    data = request.get_json() or {}
    text = str(data.get('csv') or '')
    contacts = get_phonebook()
    existing = {str(c.get('number')) for c in contacts}
    added = 0
    for line in text.splitlines():
        ...
        if not name or not number or number in existing:
            continue
        contacts.append({
            'id': f"c_{int(time.time() * 1000)}_{added}",
            'name': name,
            'number': number,
    ...
    save_phonebook(contacts)
    sync_asterisk_directory()
    auth_mgr.audit(session.get('username'), 'phonebook_import', f'{added} added')
    return jsonify({'status': 'ok', 'added': added, 'contacts': contacts})


@app.route('/api/phonebook/export', methods=['GET'])
def api_phonebook_export():
    contacts = get_phonebook()
    lines = ['name;number;company;email']
    for c in contacts:
        lines.append(';'.join([
            str(c.get('name', '')).replace(';', ' '),
            str(c.get('number', '')),
            str(c.get('company', '')).replace(';', ' '),
            str(c.get('email', '')),
        ]))
    csv_data = '\n'.join(lines)
    resp = make_response(csv_data)
    resp.headers['Content-Type'] = 'text/csv; charset=utf-8'
    resp.headers['Content-Disposition'] = 'attachment; filename=phonebook.csv'
    return resp
...
# ================= CONFERENCES (MEETME) =================
CONFERENCES_CONF = '/etc/asterisk/meetme.conf'
MEETME_ADMIN_PIN_DEFAULT = '9999'


def get_conferences():
    cfg = load_integrations()
    return cfg.get('conferences', [])


def save_conferences(rooms):
    cfg = load_integrations()
    cfg['conferences'] = rooms
    save_integrations(cfg)
    generate_meetme_conf()
    generate_dialplan_from_tree()


def generate_meetme_conf():
    rooms = get_conferences()
    out = [
        "; ==========================================================",
        "; Conference rooms (Auto-generated by Asterisk PBX Web)",
        "; ==========================================================",
        "[rooms]",
    ]
    for r in rooms:
        ...
        if not number:
            continue
        pin = str(r.get('pin', '') or '0')
        admin_pin = str(r.get('admin_pin', '') or '')
        out.append(f"; Комната: {r.get('name', number)}")
        out.append(f"conf => {number},{pin},{admin_pin},")
    out.append("")
    try:
        with open(CONFERENCES_CONF, 'w', encoding='utf-8') as f:
            f.write('\n'.join(out))
        run_asterisk('meetme reload')
    except Exception as e:
        print(f'[conferences] save failed: {e}')
    return True


@app.route('/api/conferences', methods=['GET'])
def api_conferences_list():
    rooms = get_conferences()
    # Live participant counts via CLI
    for r in rooms:
        number = str(r.get('number', ''))
        raw = run_asterisk(f'meetme list {number} concise') or ''
        r['participants'] = len([l for l in raw.splitlines() if l.strip() and 'No active' not in l and 'Conference' not in l])
    return jsonify({'status': 'ok', 'conferences': rooms})


@app.route('/api/conferences/save', methods=['POST'])
def api_conferences_save():
    data = request.get_json() or {}
    cid = str(data.get('id') or '').strip() or f"conf_{int(time.time())}"
    name = str(data.get('name') or '').strip()
    number = str(data.get('number') or '').strip()
    if not name or not number:
        return jsonify({'status': 'error', 'message': 'Название и номер конференции обязательны'})
    entry = {
        'id': cid,
        'name': name,
        'number': number,
        ...
        'music_on_hold': str(data.get('music_on_hold') or '').strip(),
        'record': bool(data.get('record')),
        'mute_join': bool(data.get('mute_join')),
        'announce_join': bool(data.get('announce_join', True)),
    }
    rooms = get_conferences()
    updated = False
    for i, r in enumerate(rooms):
        if r.get('id') == cid or str(r.get('number')) == number:
            rooms[i] = entry
            updated = True
            break
    if not updated:
        rooms.append(entry)
    save_conferences(rooms)
    auth_mgr.audit(session.get('username'), 'conference_save', f'{name} ({number})')
    return jsonify({'status': 'ok', 'conference': entry, 'conferences': rooms})


@app.route('/api/conferences/delete', methods=['POST'])
def api_conferences_delete():
    data = request.get_json() or {}
    cid = str(data.get('id') or '')
    rooms = [r for r in get_conferences() if r.get('id') != cid]
    save_conferences(rooms)
    auth_mgr.audit(session.get('username'), 'conference_delete', cid)
    return jsonify({'status': 'ok', 'conferences': rooms})
...
def list_sound_files():
    items = []
    for d in (SOUNDS_DIR, '/var/lib/asterisk/sounds'):
        if not os.path.isdir(d):
            continue
        for f in sorted(os.listdir(d)):
            if f.lower().endswith(('.wav', '.g722', '.ulaw', '.alaw', '.gsm', '.mp3', '.sln')):
                p = os.path.join(d, f)
                try:
                    items.append({
                        'filename': f,
                        'category': 'custom' if d == SOUNDS_DIR else 'system',
                        'folder': d,
                    ...
                    pass
    # De-dup by filename, custom wins
    seen = {}
    for it in items:
        if it['filename'] not in seen or it['category'] == 'custom':
            seen[it['filename']] = it
    return sorted(seen.values(), key=lambda x: (x['category'] != 'custom', x['filename']))
...
def api_sounds_list():
    ...
    q = (request.args.get('q') or '').strip().lower()
    if q:
        files = [f for f in files if q in f['filename'].lower()]
...
def api_sounds_upload():
    ...
    if ext not in ('.mp3', '.wav', '.gsm', '.g722', '.ulaw', '.alaw', '.sln'):
        return jsonify({'status': 'error', 'message': 'Недопустимый формат'})
    try:
        os.makedirs(SOUNDS_DIR, exist_ok=True)
        safe = secure_filename(uploaded.filename) or f"prompt_{int(time.time())}{ext}"
        target = os.path.join(SOUNDS_DIR, safe)
        uploaded.save(target)
        final = safe
        if ext == '.mp3':
            wav_name = os.path.splitext(safe)[0] + '.wav'
            wav_path = os.path.join(SOUNDS_DIR, wav_name)
            try:
                subprocess.run(['sox', target, '-r', '8000', '-c', '1', wav_path],
                ...
                pass
        auth_mgr.audit(session.get('username'), 'sound_upload', final)
        return jsonify({'status': 'ok', 'filename': final})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)})


@app.route('/api/sounds/<path:filename>')
def api_sounds_serve(filename):
    safe = os.path.basename(filename)
    if os.path.exists(os.path.join(SOUNDS_DIR, safe)):
        return send_from_directory(SOUNDS_DIR, safe)
    for d in ('/var/lib/asterisk/sounds',):
        if os.path.exists(os.path.join(d, safe)):
...
def api_sounds_delete():
    ...
    if not filename or not os.path.exists(path):
        return jsonify({'status': 'error', 'message': 'Файл не найден (системные файлы удалять нельзя)'})
    try:
        os.remove(path)
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)})
    auth_mgr.audit(session.get('username'), 'sound_delete', filename)
    return jsonify({'status': 'ok'})
...
def generate_pjsip_conf(accounts=None):
    ...
    for acc in accounts:
        ...
        vm_boxes = {b.get('exten'): b.get('context', 'default') for b in get_voicemail_boxes()}
        if str(ext) in vm_boxes:
            out.append(f"mailboxes={ext}@{vm_boxes[str(ext)]}")
...
def generate_dialplan_from_tree():
    ...
    # ACD Queues (real Asterisk queues from queues.conf)
    queue_dialplan_lines = []
    for q in get_queues():
        q_ext = str(q.get('exten', '')).strip()
        if not q_ext:
            continue
        q_name = q.get('name', q_ext)
        queue_dialplan_lines.append(f"; ACD Очередь: {q_name} ({q_ext})")
        queue_dialplan_lines.append(f"exten => {q_ext},1,NoOp(Входящий в очередь: {q_name} -> {q_ext})")
        queue_dialplan_lines.append(f" same => n,Set(REC_FILE=${{STRFTIME(${{EPOCH}},,%Y%m%d-%H%M%S)}}_${{CALLERID(num)}}_queue{q_ext}.wav)")
        queue_dialplan_lines.append(" same => n,Set(__CALL_ID=${UNIQUEID})")
        queue_dialplan_lines.append(" same => n,Set(__CALL_SRC=${CALLERID(num)})")
        queue_dialplan_lines.append(f" same => n,Set(__CALL_DST={q_ext})")
        queue_dialplan_lines.append(" same => n,Set(__CALL_DIRECTION=inbound)")
        queue_dialplan_lines.append(" same => n,Set(__REC_PATH=${RECORD_DIR}/${REC_FILE})")
        queue_dialplan_lines.append(" same => n,Set(CHANNEL(hangup_handler_push)=sub-post-call-sync,s,1)")
        queue_dialplan_lines.append(" same => n,Answer()")
        queue_dialplan_lines.append(" same => n,MixMonitor(${REC_PATH},r(${REC_PATH}_rx.wav)t(${REC_PATH}_tx.wav))")
        queue_dialplan_lines.append(f" same => n,Queue({q_ext},t,,,120)")
        queue_dialplan_lines.append(" same => n,Hangup()")
    queues_dialplan_block = "\n".join(queue_dialplan_lines)

    # Conference rooms (MeetMe)
    conf_dialplan_lines = []
    for conf in get_conferences():
        c_num = str(conf.get('number', '')).strip()
        if not c_num:
            continue
        c_name = conf.get('name', c_num)
        opts = 'A' if conf.get('announce_join', True) else ''
        if conf.get('mute_join'):
            opts += 'M'
        conf_dialplan_lines.append(f"; Конференция: {c_name} ({c_num})")
        conf_dialplan_lines.append(f"exten => {c_num},1,NoOp(Конференция: {c_name} -> {c_num})")
        if conf.get('record'):
            conf_dialplan_lines.append(" same => n,Set(CONF_REC=${RECORD_DIR}/${STRFTIME(${EPOCH},,%Y%m%d-%H%M%S)}_conf" + c_num + ".wav)")
            opts += 'r'
        conf_dialplan_lines.append(" same => n,Answer()")
        conf_dialplan_lines.append(f" same => n,MeetMe({c_num},{opts})")
        conf_dialplan_lines.append(" same => n,Hangup()")
    conf_dialplan_block = "\n".join(conf_dialplan_lines)
...
"""
 ...
 same => n,Hangup()"""
                    ...
                    vm_ctx = 'default'
                    vm_mail = '101'
                    vm_id = target.replace('Voicemail', '').replace('(', '').replace(')', '').strip()
                    if '@' in vm_id:
                        vm_mail, vm_ctx = vm_id.split('@', 1)
                    elif vm_id:
                        vm_mail = vm_id
                    act_line = f""" same => n,Answer()
 same => n,Wait(1)
 same => n,VoiceMail({vm_mail}@{vm_ctx},u)
 same => n,Hangup()"""
                else:
...
; 3. Голосовая почта: *[номер] → VoiceMailMain, [номер] занят/не ответил → голосовая почта
exten => _*[1-9]XX,1,NoOp(Голосовая почта: доступ ${{CALLERID(num)}} -> ${{EXTEN}})
 same => n,Answer()
 same => n,Wait(1)
 same => n,VoiceMailMain(${{EXTEN:1}}@default,s)
 same => n,Hangup()

exten => _*[1-9]XXX,1,NoOp(Голосовая почта: доступ ${{CALLERID(num)}} -> ${{EXTEN}})
 same => n,Answer()
 same => n,Wait(1)
 same => n,VoiceMailMain(${{EXTEN:1}}@default,s)
 same => n,Hangup()

; 4. Телефонная книга: *650 — поиск абонента по имени (Asterisk Directory)
exten => 650,1,NoOp(Телефонная книга: поиск по имени от ${{CALLERID(num)}})
 same => n,Answer()
 same => n,Wait(1)
 same => n,Directory(default)
 same => n,Hangup()
{groups_dialplan_block}
